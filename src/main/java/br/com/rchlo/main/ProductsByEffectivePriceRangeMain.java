...
public class ProductsByEffectivePriceRangeMain {
    ...
    public static void main(String[] args) {
        ...
        List<Product> filteredProducts = productsByEffectivePriceRangeFilter.filter(minimumPrice, maximumPrice, allProducts);

        for (Product product : filteredProducts) {
            System.out.printf("%s - %s - R$ %.2f %n", product.getCode(), product.getName(), product.getEffectivePrice());
        }
